"""
GenVAD v4_convlstm — Future Frame Prediction cu ConvLSTM.

Comparabil cu v4_future (U-Net): aceleași date, scor MSE, Micro/Macro AUC.

Colab UCSD:
  !python v4_convlstm/main.py --dataset ucsd --data_path /content/UCSD_Dataset \\
      --output_dir /content/v4_convlstm_ucsd --device cuda

Colab Avenue:
  !python v4_convlstm/main.py --dataset avenue --data_path /content/Avenue_Dataset \\
      --output_dir /content/v4_convlstm_avenue --device cuda
"""
...
from model.convlstm_predictor import FutureFrameConvLSTM
...
def main():
    ...
    print(
        f"ConvLSTM: hidden={args.hidden_channels}, layers={args.num_layers}, "
        f"params={n_params:,}"
    )
    ...
    if args.run_type in ("train", "train_eval"):
        ...
        for epoch in range(start_epoch, args.epochs):
            avg_loss = train_one_epoch(
                model, train_loader, optimizer, device, epoch, args.print_freq
            )

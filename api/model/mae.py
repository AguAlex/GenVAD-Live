...
class MaskedAutoencoderCvT(nn.Module):
    ...
    def per_patch_squared_error(self, imgs: torch.Tensor, pred: torch.Tensor) -> torch.Tensor:
        """Eroare medie pătratică pe fiecare patch, formă (B, h_p, w_p)."""
        imgs_p = self.patchify(imgs)
        err = (imgs_p - pred).pow(2).mean(dim=-1)
        p = self.patch_embed.patch_size[0]
        h = imgs.shape[2] // p
        w = imgs.shape[3] // p
        return err.view(imgs.shape[0], h, w)
